...
function RecursoChecklist({ recursos, selected, onToggle }) {
  ...
  return (
    <div>
      <div className="table-search" style={{ marginBottom: 10 }}>
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar recurso disponible…" />
      </div>
      <div style={{ border: "1px solid var(--color-border)", borderRadius: "var(--radius-sm)", maxHeight: 220, overflowY: "auto" }}>
        {filtrados.length === 0 && (
          <div style={{ padding: 16, fontSize: 13, color: "var(--color-text-secondary)" }}>
            No hay recursos disponibles que coincidan.
          </div>
        )}
        {filtrados.map((r) => (
          <label
